function Schedule(CONFIG) {
    ...
    schedule.makeSessionItemTemplateData = function(sessionItem, expanded) {
        var templatedata = {
            ...
            sessionClass: sessionItem.everyone ? 'everyone' : sessionItem.length == '60 minutes' ? 'length-short' : 'length-long',
            showDay: false,
            showFacilitators: false,
            smartypants: schedule.smartypants
        }
        if (sessionItem.talk) {
            templatedata.sessionClass = 'everyone highlight';
        }
    ...
    schedule.loadChosenTab = function() {
        ...
        if (schedule.chosenTab == 'favorites') {
            ...
            schedule.addCaptionOverline('<p class="overline"><i class="fa fa-cc"></i> indicates sessions with <a href="https://srccon.org/transcription/">live transcription</a> available</p>');
